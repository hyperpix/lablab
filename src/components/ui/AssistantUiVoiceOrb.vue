...
void main() {
  ...
  // Adjust UV coordinate to ensure circle remains perfectly circular regardless of aspect ratio
  if (u_aspect > 1.0) {
    uv.x *= u_aspect;
  } else {
    uv.y /= u_aspect;
...
let ctx: ReturnType<typeof initWebGL> = null
const startTime = ref(performance.now())
const currentParams = ref({ ...STATE_PARAMS.idle })
const targetParams = ref({ ...STATE_PARAMS.listening })

watch(
  () => props.state,
  (newState) => {
    targetParams.value = { ...STATE_PARAMS[newState] }
  },
  { immediate: true }
)
...
const render = () => {
  ...
  // Modulate amplitude by live audio level when listening or speaking
  const audioBoost = props.audioLevel > 0.01 ? props.audioLevel * 0.35 : 0
  const isAudioReactive = props.state === 'listening' || props.state === 'speaking'
  const finalAmplitude = isAudioReactive ? p.amplitude + audioBoost : p.amplitude
  ...
  gl.uniform1f(uniforms.u_amplitude, finalAmplitude)
  ...
  gl.uniform3fv(uniforms.u_color0, new Float32Array(colors[0]))
  gl.uniform3fv(uniforms.u_color1, new Float32Array(colors[1]))
  gl.uniform3fv(uniforms.u_color2, new Float32Array(colors[2]))
...
<template>
  <div class="relative w-full h-full aspect-square flex items-center justify-center">
    <canvas 
      ref="canvasRef" 
      class="w-full h-full aspect-square block"
    ></canvas>
  </div>
</template>
